...
避免服务上线即死亡换句话说就是项目上线之后还能迭代进化，所以必须借助AI协助，要让AI参与到长期的迭代中来，管是修复一个BUG、调整一段prompt、或者增加一个新功能，都必须让AI知道这个项目在做什么，原来做过哪些决定等等。那么就需要一个中枢来提供知识和想法。

这本质就是今年流行的 harness 思想，虽然各家厂商都有各自的 harness 定义，有的狭义有的广义，我的总结是一句话：
...
runbook是一个PRD中枢衍生出来的小文档，解决什么问题呢，就是暴露出运行状态数据源（接口或者数据表）之后，开发者自己的agent去读取查看没问题了，但是你的服务肯定不止一个读者，你的项目的理论要报告给同事和领导，这时候就需要一个runbook把他们关注的数据源部分做一个引导，说清楚怎么怎么连接。
...
!!! warning 注意L1中还要包含异常事件
    可以想成是接入sentry，用来记录所有run运行的异常和run之外的异常，包括run之前就报错，一些cron任务，甚至是观测链路自己本身。具体实现的话可以直接用sentry，也可以单独建一个表来存放，让异常变成一等公民。



> AI 批注：第一条里"特别是**链路中**出现的失败和异常"是全文唯一提到异常收集的地方，而"链路中"三个字把范围隐式限定在了 run 的执行过程内。loop_demo 忠实实现了这句话，于是留下三个实测盲区：① 取 prompt 在 run 行创建之前就抛异常，代码注释自己承认"连 lineage 都无法完整记录"——依赖挂一小时、期间全部 502，事后从 L1 查不到丢了多少请求，fail_rate 纹丝不动；② 任意端点的未预期 500 走 uvicorn 的标准库 logging，连结构化日志文件都进不去，"拿 X-Request-Id 反查 run"的排查路径对它是断链；③ 报告链路自己挂了只会写进 /tmp 的 cron 日志。建议把这句升级成一条原则：**每一个 raise 都必须有一个可查询的 L1 落点**——run 内的进 lineage 的 error 列，run 外的进异常事件流；并配一条可验收的不变量：凡是返回给调用方的 5xx，事后必须能凭 request_id 从 L1 接口查到。
...
> AI 批注：表中"一次请求（request），落到底层仍是 run"是全文最关键的隐含假设，而它在在线服务形态不成立：不是每个请求都会产生 run（run 创建前就失败的请求），也不是每个异常都有 request（启动失败、cron 拉起的报告链路）。批处理形态下 run 包络≈整个进程，这个洞基本不张开——大概也是它被漏掉的原因；但"闭环的资产是 lineage 设计本身，跟形态无关"要成立，L1 就得把 run 之外的异常也收进来。
