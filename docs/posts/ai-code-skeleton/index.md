...
比如最复杂的 pipeline 提交接口，骨架里是这样（高亮的是文件级那层）：
...
会报错的检查，代替会烂的注释。能变成检查的约束，别停在注释。两个落地：

- "类型必须和前端对齐" → `Pydantic → openapi.json → 生成 TS` 流水线加一个测试。
- "骨架与副本一致" → `check_skeleton_sync` 脚本：比对文件 / 类 / 公共函数存在性、函数签名、docstring **逐字**、实码文件整文件；`#` 注释忽略，`_` 开头私有 helper 允许副本多出。挂 pre-commit / CI，docstring 漂移就从迟早发生的事变成一条 exit 1。
