...
上面写完没多久，在同一个系统的另一半（礼包推送的 BI 后端）真把两个方向接到了一起：老仓库要重构，先逆向出骨架读懂它，然后发现重构设计可以直接画在这份骨架上，最后干脆让骨架长期活下来，当整个项目的设计蓝图。用法沉淀成两套：怎么读它（骨架阅读法），怎么拿它开发（骨架开发法）。

### 骨架阅读法：给每句话标上信任方向

前面说逆向骨架最危险的是"把 AI 的不确定性洗成一段可信的话"，解法是挂行号。实践里这条纪律扩成了两个标记——因为一份承载重构的骨架里，断言和指令是混在一起的：

- `[源: 文件:行号]` —— 对老代码的**转述**。语义沿用老实现，读的时候可跳回源码核对，实装的时候回老仓库对着行号搬码。
- `[新: 日期 一句话]` —— 本次重构的**前向设计**。没有老码可搬，docstring 就是规约，照着从零实现。

一份骨架从此同时是地图和 spec，每段说明的信任方向逐条标注，不用靠上下文猜"这句该信还是该查"。实装完成后标记随手删掉；历次设计变更则写进 README 的**决策日志**（只增不改）——第四批方案当天被第五批推翻这种事，日志里留轨迹，骨架里只留最终态。

### 骨架开发法：蓝图先行

开工不直接在骨架上改，而是复制一份出去，从此三个目录角色固定：
...
2. 改骨架：签名 + docstring 写规约；设计思考写成 # 注释（留在骨架）
3. 人批准后，同步到工作副本：写函数体，docstring 逐字照搬，# 注释不带过去
4. 跑对齐检查，必须通过
```

这正面回应了开头那个困扰——"AI 写了一个大仓库，读起来成本高，感觉失去了对项目的控制"。控制点就是骨架：代码量小到人能通读，改起来不牵一发动全身；工作副本只是骨架的投影，AI 在副本里写多少行，都不影响你手里握着的那张图。

### docstring = 规约，`#` 注释 = 思考

前面"该删该留"的判断标准，这次固化成了**语法级**的分界。判据一句话：**使用者 / 读者需要的 → docstring；实现者 / 考古需要的 → `#` 注释。**

- **docstring**：职责、参数/返回/Raises 语义、调用方必须遵守的约束——骨架与副本**逐字一致**，只放稳定规约。
- **`#` 注释**：`[源:]`/`[新:]` 标记、设计理由（为什么不选 X）、踩坑日期、实现草图——**只写在骨架**；到了副本里，草图变成真函数体，思考留在蓝图里供考古。

一个 must-do 警告怎么拆的例子：

```python
def get_pushable_packages(...) -> list[CustomIap]:
    """pay2 侧应可见的**全部**礼包；每次推送必须列全，不能只推增量。"""  # ← 规约
    # 2026-07-12 dev 实测：单推 1 条会硬删该角色其余在售礼包（audience 全量替换）  # ← 理由
    ...
```

### 会报错的检查，代替会烂的注释

前面说"能悄悄烂掉的注释，永远不如会大声报错的检查"，这句也落了地：一个 `check_skeleton_sync` 脚本比对骨架与副本——文件 / 类 / 公共函数存在性、函数签名、docstring **逐字**、实码文件整文件；`#` 注释忽略，`_` 开头私有 helper 允许副本多出。挂上 pre-commit / CI，"docstring 漂移"就从迟早发生的事变成一条 exit 1。

"类型层写实"的原则也顺势延伸：不止类型，配置加载、表定义这两个文件在骨架里就是**可运行实码**，副本整文件照搬，改动两边同步——契约当真，行为留空，和第一天的分界一脉相承。

### 文档也分骨架和血肉

一个没预料到的副产品：围绕骨架的文档自己长出了寿命分工——

- **交接文档（HANDOVER）**：一次性。开工用完就删，存活的内容（已定决策、没做完的外部待办）并回 README。
- **README**：长期。回答"如何从骨架复现一个完整项目"——架构图、已定决策、分步实施顺序（每步带验收），外加只增不改的决策日志。
- **CLAUDE.md**：最简。它是每次 AI 会话都自动加载的那份，所以只放"这个目录是什么 + 四步工作流 + 三条硬规则"，短到不可能被略读。

最后这份 CLAUDE.md 全文也就三十行：
...
- [ ] 骨架承载重构时，用 `[源: file:line]` / `[新: 日期]` 双标记区分转述与设计
- [ ] 骨架要长期当蓝图：复制出工作副本开工，蓝图只读；改动蓝图先行，人批准后再同步副本
- [ ] docstring = 规约（两边逐字一致），`#` 注释 = 思考（只留骨架）；对齐检查脚本挂 CI 盯漂移
- [ ] 文档分寿命：交接文档一次性用完删，README 管"如何复现"，CLAUDE.md 只放最短的硬规则

## 两个方向，一句话

**正向的骨架是一份 spec，spec 的活儿在代码存在的那一刻就干完了；逆向的骨架是一张地图，地图永远不能代替你亲自走一遍地形。** 同一把工具，握法不同：正向你信它，因为它定义未来；逆向你查它，因为它转述现在。

后来多了第三种握法：**让骨架活下来当蓝图，它就成了你和 AI 之间的合同——你在骨架里握设计权，AI 在副本里干实装的活，对齐检查当公证人。**
