...
### 图片跟文字一起走 Git（git-lfs） { #副机图片 }

**文字和图片现在都走 Git。** 二进制媒体经 git-lfs 入库（机制详见[资源备份](#媒体备份)）：commit 时文件在 git 里变成几十字节的指针，push 时真身由自制 agent 自动传进备份 COS 桶。副机加图不再需要 rsync 对路径、不需要手动跑备份 —— 一次 `git push`，文章和图片全部到位：
...
只写文字、不碰图的副机可以**零密钥**：`GIT_LFS_SKIP_SMUDGE=1` clone 后直接写 —— 工作区里图片是指针文本，md 照常编辑、push 照常走，只是本地看不了图。
...
## 资源备份：自制 mini LFS，备份桶即媒体档案馆 { #媒体备份 }
...
本站的 agent 是 ~140 行 Python（[`scripts/lfs-cos-agent.py`](../assets/lfs-cos-agent.py)），收到 upload/download 事件就用 `qcloud_cos` SDK 在备份桶里存取。并发、本地缓存（`.git/lfs/objects`）、sha256 完整性校验、增量判断全由 git-lfs 客户端自带，agent 只是个哑搬运工。
...
| 备份桶 `lfs/` 前缀 | 媒体**真身**，按内容寻址存 `lfs/ab/cd/<sha256>` |
...
agent 从项目 `.env` 读凭证，所以**碰媒体的机器才需要密钥**（上传下载都要），纯文字机零密钥（见[副机接入](#副机图片)）。多机分发建议 CAM 子账号：只授权备份桶读写，主密钥永不离开主机。哪天要开放协作（让不持钥的人 clone），再把 agent 的存储层原样搬进一个签发预签名 URL 的 Batch API 服务即可，升级路径是平滑的。
...
之后每次写完文章只跑 `./deploy.sh` 一行，两个视图（HTML + md 源）同时更新，证书永久无需操心；在别的机器写的内容会被脚本第 0 步的 pull 自动捎带上线（见[日常写作流](#写作流)）。
