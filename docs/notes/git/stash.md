...
- **先撞墙，再解释**。第一步不是建仓库，是在空仓库里直接 `git stash` 吃一个 `You do not have the initial commit yet`。报错自己会解释"stash 记录的是相对某个 commit 的差异"，比先讲一遍原理记得牢。同样的还有：`stash` 之后故意让我发现未跟踪文件还赖在原地、`git stash show` 对纯 `-u` 的 stash 输出空的。**坑是安排好的，但要我自己踩到。**
- **输出先自己读**。每步 AI 只说"重点看第二条的输出，你自己找找哪里不对劲"，不直接点破。这一步逼出的问题最有价值 —— 「为什么会出现 `+功能2` 和 `-功能2`？这一行我明明没动」就是这么问出来的，答案是结尾换行符（见上面那条 tip）。
- **行为靠实测，不靠记忆**。凡是"会怎样"的问题，AI 都在临时仓库里跑对照实验再回答，不凭印象。今天这么验过三处：连续 `apply` 两次会不会叠加、有没有 `MERGE_HEAD`、缺换行符时 diff 怎么对齐。文章里凡标"实测 git 2.34.1"的都是这么来的。
- **手滑了不重来**。我手快直接 `pop` 掉了本该 `apply` 的 stash，AI 没让我重置回去，而是就着这个残局讲了 `Dropped stash@{0} (c38ce14...)` 那行 hash 的用处和 `git fsck --unreachable` 捞游离对象。**计划外的失误比设计好的步骤更接近真实事故。**
